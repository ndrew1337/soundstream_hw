...
class WandBWriter:
    ...
    def _waveform_numpy(self, audio):
    ...
    def build_audio(self, audio, sample_rate, caption=None, peak_normalize_preview=False):
        arr = self._waveform_numpy(audio)
        cap = caption or ""
        if peak_normalize_preview:
            peak = float(np.max(np.abs(arr))) + 1e-12
            arr = np.clip(arr / peak * 0.99, -1.0, 1.0).astype(np.float32)
            cap = (cap + " [peak-normalized for preview]").strip()
        return self.wandb.Audio(arr, sample_rate=sample_rate, caption=cap or None)

    def build_image(self, image_array):
        return self.wandb.Image(image_array)
    ...
    def add_audio(
        ...
        """
        aud = self.build_audio(
            audio,
            sample_rate=sample_rate,
            caption=caption,
            peak_normalize_preview=peak_normalize_preview,
        )
        self.wandb.log({self._object_name(audio_name): aud}, step=self.step)

    def log_media_ordered(self, items: dict):
        if not items:
            return
        prefixed = {
            self._object_name(str(name)): value for name, value in items.items()
        }
        self.wandb.log(prefixed, step=self.step)
